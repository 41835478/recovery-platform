...
(function () {
    ...
    UI = {
        ...
        start: function(callback) {
            ...
            // set manually
            var port = _POST_;//window.location.port;
            if (!port) {
			...
			UI.initSetting('host', _HOST_);//window.location.hostname
            UI.initSetting('port', port);
            UI.initSetting('password', _PASSWORD_);//''
            UI.initSetting('encrypt', (window.location.protocol === "https:"));
            UI.initSetting('true_color', true);
			UI.initSetting('token',  _TOKEN_);
            ...
            var autoconnect = WebUtil.getConfigVar('autoconnect', true); //zhaoyg 
			autoconnect = true;
            ...
            UI.isSafari = (get_userAgent() =='Safari');
        ...
        initRFB: function () {
            try {
                UI.rfb = new RFB({'target': $D('noVNC_canvas'),
								  ...
								  'onUpdateVncname': UI.updateVncname,
        ...
        toggleClipboardPanel: function() {
            ...
            if (UI.clipboardOpen === true) {
                $('#noVNC_clipboard').fadeOut();
                UI.clipboardOpen = false;
            } else {
                $('#noVNC_clipboard').fadeIn();
        ...
        toggleFullscreen: function() {
            ...
            if (screen.fullscreenElement || // alternative standard method
                screen.mozFullScreenElement || // currently working methods
                screen.webkitFullscreenElement ||
                screen.msFullscreenElement) {
        ...
        updateFullscreenButton: function() {
            ...
            if (screen.fullscreenElement || // alternative standard method
                ...
                screen.webkitFullscreenElement ||
                screen.msFullscreenElement ) {
				$('#fullscreenButton').removeClass();
				$('#fullscreenButton').addClass('glyphicon glyphicon-resize-small');
				
            } else {
                $('#fullscreenButton').removeClass();
				$('#fullscreenButton').addClass('glyphicon glyphicon-resize-full');
            }
        },
        ...
        openSettingsMenu: function() {
			...
			$('#noVNC_settings').fadeIn();
            UI.settingsOpen = true;
        },

        // Close menu (without applying settings)
        closeSettingsMenu: function() {
			$('#noVNC_settings').fadeOut();
        ...
        setMouseButton: function(num) {
            ...
            for (var b = 0; b < blist.length; b++) {
                var button = $('#noVNC_mouse_button' + blist[b]);
                if (blist[b] === num) {
                    button.show();
                } else {
                    button.hide();
		...
		updateVncname: function(rfb_name) {
			
			var rfb_name=rfb_name.substring(rfb_name.indexOf("(")+1,rfb_name.indexOf(")"));
			
			$D('vnc_name').innerHTML = rfb_name;
		},
        ...
        updateState: function(rfb, state, oldstate, msg) {
            ...
            switch (state) {
                ...
                case 'password':
                    ...
                    $D('noVNC_connect_button').value = "设置密码";
            ...
            if (typeof(msg) !== 'undefined') {
				...
				$('#noVNC-control-bar').fadeIn("slow");
				
				$D('noVNC-control-bar').style.left = window.innerWidth/2 -
                    parseInt(window.getComputedStyle($D('noVNC-control-bar')).width)/2 + "px";
				
				setTimeout(function() {
					$('#noVNC-control-bar').fadeOut("slow");
				}, 3000)
            }
        ...
        connect: function() {
            ...
            if ((!host) || (!port)) {
				UI.updateState(null, 'fatal', null, '必须设置主机和端口');
				return false;
               // throw new Error("必须设置主机和端口");
            }
        ...
        updateViewDrag: function(drag) {
            ...
            var vmb = $('#noVNC_view_drag_button');
            ...
            if (UI.rfb_state === 'normal' &&
                ...
                if (UI.rfb.get_viewportDrag) {
					...
					vmb.removeClass();
                    vmb.addClass("iconfont icon-yidong");
            ...
            if (typeof(drag) !== "undefined" && typeof(drag) !== "object") {
				vmb.removeClass();
                if (drag) {
					vmb.addClass("iconfont icon-yidong text-glay");
        ...
        showKeyboard: function() {
            ...
            if(UI.keyboardVisible === false) {
				...
				skb.addClass("iconfont icon-jianpan1 text-glay");
            } else if(UI.keyboardVisible === true) {
                kbi.blur();
				skb.addClass("iconfont icon-jianpan1");
        ...
        keepKeyboard: function() {
            ...
            if(UI.keyboardVisible === true) {
                ...
                $D('showKeyboard').className = "iconfont icon-jianpan1 text-glay";
            } else if(UI.keyboardVisible === false) {
                $D('keyboardinput').blur();
                $D('showKeyboard').className = "iconfont icon-jianpan1";
            }
        },
        ...
        showExtraKeys: function() {
            UI.keepKeyboard();
			$('#showExtraKeysButton').removeClass();
			
			if (typeof UI.extraKeysVisible=='undefined') {
				UI.extraKeysVisible = false;
			}

            if(UI.extraKeysVisible === false) {
                $('#toggleCtrlButton').show();
                $('#toggleAltButton').show();
                $('#sendTabButton').show();
                $('#sendEscButton').show();
                $('#showExtraKeysButton').addClass("iconfont icon-shoushi text-glay");
                ...
                $('#sendTabButton').show();
                $('#sendEscButton').show();
                $('#showExtraKeysButton').addClass("iconfont icon-shoushi");
                UI.extraKeysVisible = false;
            }
        },
        ...
        toggleCtrl: function() {
            UI.keepKeyboard();
			if (typeof UI.ctrlOn=='undefined') {
				UI.ctrlOn = false;
			}
            if(UI.ctrlOn === false) {
                UI.rfb.sendKey(XK_Control_L, true);
                $D('toggleCtrlButton').className = "iconfont icon-ctrl text-glay";
                UI.ctrlOn = true;
            } else if(UI.ctrlOn === true) {
                UI.rfb.sendKey(XK_Control_L, false);
				
                $D('toggleCtrlButton').className = "iconfont icon-ctrl";
                UI.ctrlOn = false;
            }
        },
        ...
        toggleAlt: function() {
            UI.keepKeyboard();
			if (typeof UI.altOn=='undefined') {
				UI.altOn = false;
			}
            if(UI.altOn === false) {
                UI.rfb.sendKey(XK_Alt_L, true);
                $D('toggleAltButton').className = "iconfont icon-alt text-glay";
                UI.altOn = true;
            } else if(UI.altOn === true) {
                UI.rfb.sendKey(XK_Alt_L, false);
                $D('toggleAltButton').className = "iconfont icon-alt";
                UI.altOn = false;
            }
        },
        ...
        setBarPosition: function() {
            //$D('noVNC-control-bar').style.top = (50) + 'px';
...
function sendCtrlAltFN(f) {
	...
	UI.rfb.sendKey(0xFFE3, 'down');
	UI.rfb.sendKey(0xFFE9, 'down');
	UI.rfb.sendKey(keys_code[f], 'down');
	UI.rfb.sendKey(keys_code[f], 'up');
	UI.rfb.sendKey(0xFFE9, 'up');
	UI.rfb.sendKey(0xFFE3, 'up');
};
