...
class Index extends Admin {
	...
	public function index() {
		...
		$exclusivedevice = $this->device->getExclusiveDevice('cdp,sb',array('status'=>1,'attribute'=>1));
...
//			$sanp_list[date('Ymd',$vo['create_time'])]['data'][]=$vo;
	   ...
	   if(IS_POST){
		   ...
		   $data['system_type'] = input('system_type')=='Windows'? 1 : 0;
            ...
            if(is_liter_system()){  // 如果是轻量版
               ...
               if( $this->add_virtual_common($data,$snap,$spath,$storage_id,$group_id,$keli_id,$data['system_type'])){
				   ComputeVirtual::addVirtualLog($data['name'],$data['device_id'],$data['type'],1);
				   return $this->success(lang('create').lang('success'));
			   }else{
   ...
   private function add_virtual_liter($data,$snap,$spath,$storage_id,$system_type){
	...
	private function send_create_keli_message($keli_id){
		...
		$data['nId'] = $keli_id;
		$result = \Netmessage::send_message('W2M_Create_Small_Snap',$data);
		if($result['code'] == 1){
			return true;
		}else{
			return false;
...
//		w2m_start_cdp 开启
		if ($status == 1)
			$message_str = 'w2m_start_cdp';
		else if($status == 2)
        	$message_str = 'w2m_pause_cdp';
		else if($status == 3)
			$message_str = 'w2m_stop_cdp';
	...
	public function get_clone_state(){
		...
		$result = \Netmessage::send_message('w2m_update_cloning_info', $data);
...
//		$end_min = explode(':',$end)[1];
	...
	private  function send_message($id,$type){
		...
		if($type == 1){
			$message_str = 'w2m_create_snap';

		}
		if($type == 2){
			$message_str = 'w2m_merge_snap';
		}
        if($type == 3){
			$message_str = 'w2m_delete_cdp';
		}
