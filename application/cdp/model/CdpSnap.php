...
class CdpSnap extends Model
{
    ...
    public function del($task_id,$sub_task_id){
        ...
        $this->snap_data = $this->field('id,harddisk_id,file_name,group_id,storage_id,create_time')
        ...
        $this->begin_time = $this->snap_data[0]['create_time'];
        $this->end_time = end($this->snap_data)['create_time'];
...
//				$data[$v['group_id']]['virtual_count'] = 0;
    ...
    private function handle_keli_available($time,$data){
        ...
        //如果小于三个，查看是否有当前点
        $map['type'] = 2;
        $current_snap = $this->field('id,create_time')->where($map)->order('id asc')->select();
        // 如果不存在，则不做处理
        if(!$current_snap){
            return $data;
        }
        ...
        if($snap_count==2){
            ...
            $last_snap['begin_time'] = $last_snap['create_time'];
        ...
        if($snap_count==1){
            ...
            if(count($data)==1){
                ...
                $data[0]['available'] = 1;
                $data[0]['begin_time'] = $data[0]['create_time'];
                ...
                $last_two_snap['begin_time'] = $last_two_snap['create_time'];
                $last_two_snap['end_time'] =  $last_snap['create_time']-1;
