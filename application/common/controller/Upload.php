...
class Upload {
	...
	public function upload() {
		...
		if($config['exts']){
			$check_exts = $file->checkExt($config['exts']);
			if(!$check_exts){
				$ext = implode(',', $config['exts']);
				$return['status'] = 0;
				$return['info']   = '请上传正确格式【'.$ext.'】的文件';
	...
	public function save($config, $type, $file) {
		...
		if($type == 'images'){
			$dbname = 'picture';	
		}elseif($type == 'cloud' || $type == 'system'  || $type == 'package' ){
			$dbname = 'goods_file';
		}elseif($type == 'wechat'){
			$dbname = 'mp_picture';	
		}else{
