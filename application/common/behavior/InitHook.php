...
class InitHook {
	...
	public function run(&$request) {
		//未安装时不执行
		if (substr(request()->pathinfo(), 0, 7) != 'install' && is_file(APP_PATH . 'install.lock')) {
			//初始化某些配置信息
	...
	protected function setRoute() {
		...
		foreach ($list as $key => $value) {
			$route["cms/" . $value['name'] . "/index"]  = "cms/content/index?model_id=" . $value['id'];
			$route["cms/" . $value['name'] . "/add"]    = "cms/content/add?model_id=" . $value['id'];
			$route["cms/" . $value['name'] . "/edit"]   = "cms/content/edit?model_id=" . $value['id'];
			$route["cms/" . $value['name'] . "/del"]    = "cms/content/del?model_id=" . $value['id'];
			$route["cms/" . $value['name'] . "/status"] = "cms/content/status?model_id=" . $value['id'];
