...
class Manage extends Admin
{
	...
	public function get_device_lists($group_id = 1) {
		...
		foreach ($list as &$v) {
			...
			$v['system_type'] = $this->get_system_type($v['id']);
			if($v['attribute'] == 1){
	...
	private function get_system_type($id){
		//获取操作系统
		$system_info = db('device')->where(array('id'=>$id))->field(array('system_info'))->find();
		$system_info = json_decode($system_info['system_info']);
...
//			$save_data['compute_info'] = json_encode($compute_info);
            ...
            $this->send_update_compute_message($compute_id,0);
			if ($compute_id) {			
				//记录行为
...
//			}
            ...
            $this->send_update_compute_message($id,0);
			if (false !== $reuslt) {
...
//		}
    ...
    public function update_compute(){
        ...
        if($this->send_update_compute_message($compute_id))
        {
    ...
    private function send_update_compute_message($id,$is_update=1){
       if ($is_update){
           $message_str = 'w2m_update_libvirt_computer_info';
       }else{
           $message_str = 'w2m_add_libvirt_computer';
       }
        $data['computerId'] = $id;
...
//
	...
	public function virtual_edit_type(){
		...
		$virtual = db('compute_virtual')->where(array('id'=>$virtual_id))->field(array('virtual_info'))->find();
		$virtual_info = json_decode($virtual['virtual_info'],true);
		...
		if(!VirtualManager::is_virtual_exist($name,$ip)){ //如果真实的虚机不存在，则直接删除记录
			...
			if(VirtualManager::del_virtual($name,$ip)){
				...
				if($check_data['type']==3){
					//TODO:: 删除 remote_snap表中数据及temp文件
					$res = db("remote_snap")->where(array('virtual_id'=>$id))->delete();
...
//		$this->assign('computing_data',$computing_data);
   ...
   //更新软件
	public function update(){
		...
		$result = \Netmessage::send_message('W2M_Update_Module', $data);
	...
	public function uninstall(){
		...
		$result = \Netmessage::send_message('w2m_remove_module', $data);
		if ($result['code'] == 1) {
   ...
   public function soft_status(){
	   ...
	   $softName= input('name');
	   $status = input('status');
	   $message_name = $status==1?'w2m_start_module':'w2m_stop_module';
	...
	public function soft_restart(){
		...
		$result = \Netmessage::send_message('w2m_restart_module', $data);
